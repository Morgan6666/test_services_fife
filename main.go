...
type Drugs struct {
	...
	Name      string `json:"Class chemical molecules"`
}
...
func handleRequests() {
	...
	log.Fatal(http.ListenAndServe(":8085", mainRouter))
...
func main() {
	...
	meta = []Drugs{
		Drugs{Id: "1", Structure: "O=C2/N=C(\\Nc1n(cnc12)COCCO)N", Name: "aciclovir"},
		Drugs{Id: "2", Structure: " CN(C)CCOC(C1=CC=CC=C1)C1=CC=CC=C1", Name: "diphenhydramine"},
		Drugs{Id: "3", Structure: "CCN(CC)C(=O)[C@H]1CN([C@@H]2Cc3c[nH]c4c3c(ccc4)C2=C1)C", Name: "LSD"},
	}
